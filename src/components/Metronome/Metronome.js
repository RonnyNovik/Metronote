...
class Metronome extends Component {
  ...
  playClick = () => {
    ...
    const circleElement = this.CircleContainer.current.children[
      this.state.count
    ];
  ...
  playHandler = () => {
    ...
    const CircleContainer = Object.values(
      this.CircleContainer.current.children
    );
  ...
  render() {
    //Create 4 circles
    const Circles = [1, 2, 3, 4].map(i => <Circle key={i} number={i} />);
    return (
